...
namespace Librery_MVC.Services
{
    ...
    public class LightBookService
    {
        ...
        public LightBook getLightBookById(String idBook)
        {
            ...
            while (dr.Read())
            {
                book.IdLibro = Convert.ToInt32(dr["IdLibro"]);
                book.Nombre = Convert.ToString(dr["nombre"]);
                book.AnioDeLanzamiento = Convert.ToInt32(dr["anioDeLanzamiento"]);
                book.Precio = Convert.ToDecimal(dr["precio"]);
                book.UrlImagen = Convert.ToString(dr["urlImagen"]);
                book.Estado = Convert.ToBoolean(dr["estado"]);                                  
            }
        ...
        public List<LightBook> getListLightBooks()
        {
            ...
            while (dr.Read())
            {          
                list.Add(new LightBook(Convert.ToInt32(dr["IdLibro"]), 
        ...
        public int getQuantityBook(String idBook)
        {
            ...
            if (dr.Read())
                quantity = Convert.ToInt32(dr["cantidad"]);
